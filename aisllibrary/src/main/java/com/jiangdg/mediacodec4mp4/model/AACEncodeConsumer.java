...
public class AACEncodeConsumer extends Thread {
    ...
    public void run() {
        ...
        while (!isExit) {
            if (mAudioRecord != null ) {
                ...
                int readBytes = mAudioRecord.read(audioBuf, 0, AUDIO_BUFFER_SIZE);//当前sample的byte[]大小
                if (readBytes > 0) {
                    try {
    ...
    private void startAudioRecord() {
        ...
        EncoderParams mParams = mParamsRef.get();
        bufferSizeInBytes = AudioRecord.getMinBufferSize(mParams.getAudioSampleRate(), mParams.getAudioChannelConfig(),
    ...
    boolean isAudioSatat = false;
    public byte[] audioData;

    public void startAudio(Context context) {
        try {
//            String filePath = Environment.getExternalStorageDirectory()+"/aac";
            String filePath = FileUtils.getTempPath(context) + "/aac";
            File file_tmep = new File(filePath);
            if (!file_tmep.exists()) {
                file_tmep.mkdirs();
            }
            File file = new File(filePath, "video.pcm");
            if (file.exists()) {
                file.delete();
            }
