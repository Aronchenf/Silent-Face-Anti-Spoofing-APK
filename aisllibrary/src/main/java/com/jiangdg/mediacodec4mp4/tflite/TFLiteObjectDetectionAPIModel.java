...
package com.jiangdg.mediacodec4mp4.tflite;
...
//            }
            ...
            final float[][] output = {emb, knownEmb};
            l2Normalize(output, 1e-10);
            float distance = evaluate(output);
            if (ret == null || distance > ret.second) {
                ret = new Pair<>(name, distance);
            }
        }
     ...
     * @return
     */
    private void l2Normalize(float[][] embeddings, double epsilon) {
...
//    res += "]";
        ...
        if (registered.size() > 0 && !storeExtra) {
